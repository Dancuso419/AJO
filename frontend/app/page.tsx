...
const input = "mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none";
    ...
    <main className="min-h-[80vh] w-full bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-3xl px-6 py-12">
        ...
        {groupId !== undefined ? (
          ...
          <CreateGroup onDone={id => { setView("home"); if (id !== undefined) setGroupId(id); }} onCancel={() => setView("home")} />
        ) : (
...
function Hero() {
  return (
    <div className="mb-8">
      ...
      <p className="mt-1 text-slate-600">A private rotating savings circle — amounts stay encrypted; only you can reveal your own.</p>
    </div>
  );
}
    ...
    <main className="min-h-[80vh] w-full bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-2xl px-6 py-24 text-center">
        ...
        <p className="mx-auto mt-4 max-w-md text-lg text-slate-600">
          A private rotating savings circle. Contributions and payouts are enforced on-chain, but the amounts stay encrypted —
          only you can reveal your own.
        </p>
...
function HomePanel({ onCreate, onOpen }: { onCreate: () => void; onOpen: (id: bigint) => void }) {
  ...
  if (!ajo?.address) {
    return <div className={card}><p className="text-slate-700">Switch your wallet to the <b>Sepolia</b> network to use AJO.</p></div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className={card}>
          ...
          <button className={`${primary} mt-4`} onClick={onCreate}>Create a circle</button>
        </div>
        <div className={card}>
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button className={ghost} disabled={openId === ""} onClick={() => onOpen(BigInt(openId))}>Open</button>
          </div>
          {count > 0n && (
            <div className="mt-3 flex flex-wrap gap-2">
              {Array.from({ length: Number(count) }, (_, i) => (
                <button key={i} className="rounded-lg border border-slate-300 px-2.5 py-1 text-xs hover:bg-slate-50" onClick={() => onOpen(BigInt(i))}>
                  Circle #{i}
                </button>
              ))}
            </div>
...
function CreateGroup({ onDone, onCancel }: { onDone: (id?: bigint) => void; onCancel: () => void }) {
  ...
  const order = addresses.split(/[\s,]+/).map(s => s.trim()).filter(Boolean) as `0x${string}`[];
  const valid = order.length >= 2 && order.every(a => /^0x[0-9a-fA-F]{40}$/.test(a)) && amount !== "" && duration !== "";
  ...
  return (
    <div className={card}>
      <button className="mb-4 text-sm text-slate-500 hover:text-slate-700" onClick={onCancel}>← Back</button>
      ...
      <div className="mt-5 space-y-4">
        <div>
          ...
          />
          <p className="mt-1 text-xs text-slate-500">These addresses are the members, in the order they receive the pot. Min 2.</p>
        </div>
...
function GroupDashboard({ groupId, onBack }: { groupId: bigint; onBack: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <button className="text-sm text-slate-500 hover:text-slate-700" onClick={onBack}>← All circles</button>

      <div className={card}>
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">Savings Circle #{groupId.toString()}</h2>
          <span className={`rounded-full px-3 py-1 text-xs font-medium ${g.active ? "bg-emerald-100 text-emerald-700" : g.currentRound === 0n ? "bg-amber-100 text-amber-700" : "bg-slate-200 text-slate-600"}`}>
      ...
      <div className={card}>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {g.currentRound === 0n && iAmMember && (
            <button className={primary} disabled={a.busy} onClick={a.joinGroup}>Join this circle</button>
          )}
          <button className={ghost} disabled={a.busy} onClick={() => a.mintTestTokens(1000n)}>Mint 1000 test AJOT</button>
          <button className={ghost} disabled={a.busy} onClick={a.approveAjo}>Approve AJO</button>
          {g.active && (
...
function MemberRow({ groupId, member, index, round, me }: { groupId: bigint; member: `0x${string}`; index: number; round: bigint; me?: string }) {
  const chainId = useChainId();
  const ajo = deploymentFor(ConfidentialAjo, chainId);
  const { data: joined } = useReadContract({
    address: ajo?.address, abi: ajo?.abi, functionName: "isMember", args: [groupId, member],
    query: { enabled: Boolean(ajo?.address) },
  });
  const { data: paid } = useReadContract({
    address: ajo?.address, abi: ajo?.abi, functionName: "hasPaidThisRound", args: [groupId, round, member],
  ...
  return (
    <div className="flex items-center justify-between py-2.5 text-sm">
      <div className="flex items-center gap-2">
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-xs text-slate-500">{index + 1}</span>
      ...
      <div className="flex items-center gap-3">
        <span className={`text-xs ${joined ? "text-slate-500" : "text-amber-600"}`}>{joined ? "joined" : "not joined"}</span>
        {round > 0n && (paid ? <span className="text-emerald-600">✓ paid</span> : <span className="text-slate-400">waiting</span>)}
      </div>
    </div>
  );
}
...
function LockedValue({ title, revealed }: { title: string; revealed?: bigint }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      {revealed !== undefined ? (
        <div className="mt-1 text-2xl font-bold text-slate-900">{revealed.toString()} <span className="text-sm font-normal text-slate-500">AJOT</span></div>
      ) : (
        <div className="mt-1 flex items-center gap-2 text-2xl font-bold text-slate-400">🔒 <span className="tracking-widest">••••</span></div>
      )}
    </div>
  );
}
